...
export async function crawlUOL(): Promise<NewsItem[]> {
  ...
  $("article a[href], a[href]").each((_, el) => {
    ...
    if (!existing || (!existing.image && image)) {
      found.set(url, {
        id: `uol-${Buffer.from(url).toString("base64url")}`,
        source: "UOL",
        category: "news",
        title,
...
function isArticle(url: string) {
  try {
    ...
    if (!HOSTS.has(u.hostname) || path === "/" || path.length < 10) return false;
    if (/\/(busca|search|tags?|autor|autores|colunas|videos?|podcasts?|esportes\/futebol\/times)(\/|$)/i.test(path)) return false;
...
function isUsableTitle(title: string) {
  return title.length >= 20 && title.length <= 240 && !/^(uol|home|notícias|vídeos|esportes|entretenimento)$/i.test(title);
}
...
async function enrich(item: NewsItem): Promise<NewsItem> {
  try {
    ...
    const $ = cheerio.load(html);
    const publishedAt = jsonLdDate($, "datePublished") || metaDate($, "article:published_time") || metaDate($, "datePublished") || timeDate($);
    const updatedAt = jsonLdDate($, "dateModified") || metaDate($, "article:modified_time") || metaDate($, "dateModified");
    const title = cleanText($("meta[property='og:title']").attr("content")) || cleanText($("meta[name='twitter:title']").attr("content")) || item.title;
    const subtitle = cleanText($("meta[property='og:description']").attr("content")) || cleanText($("meta[name='description']").attr("content")) || item.subtitle;
...
function pickImage($: cheerio.CheerioAPI, root: cheerio.Cheerio<any>) {
  ...
  root.find("img,source").each((_, el) => {
    ...
    if (value && !BAD_IMAGE.test(value)) {
      const absolute = absoluteUrl(value, HOME);
