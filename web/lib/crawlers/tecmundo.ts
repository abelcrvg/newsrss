...
async function crawlSite(
  ...
  articleCheck: (url: string) => boolean
): Promise<NewsItem[]> {
  ...
  for (const selector of selectors) {
    $(selector).each((_, el) => {
      ...
      if (!existing || (!existing.image && image)) {
        found.set(url, {
          id: `${sourceId}-${Buffer.from(url).toString("base64url")}`,
          source,
          category,
          title,
  ...
  const candidates = Array.from(found.values()).slice(0, MAX_ITEMS);
...
function isTecmundoArticle(url: string) {
  try {
    ...
    if (path === "/" || path.startsWith("/voxel/")) return false;
    return path.split("/").filter(Boolean).length >= 2 && !isNonArticlePath(path);
  } catch { return false; }
}
...
function isVoxelArticle(url: string) {
  try {
    const u = new URL(url);
    const path = u.pathname.toLowerCase();
    return u.hostname === HOST && path.startsWith("/voxel/") && path.split("/").filter(Boolean).length >= 2 && !isNonArticlePath(path);
  } catch { return false; }
}

function isNonArticlePath(path: string) {
  return /\/(autor|autoridade|tags|tag|busca|search|videos|galeria|lista|especial|colunistas)(\/|$)/i.test(path) || /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(path);
}

function isUsableTitle(title: string) {
  return title.length >= 20 && title.length <= 240 && !/^(home|início|mais lidas|últimas notícias|tecnologia|games)$/i.test(title);
}
