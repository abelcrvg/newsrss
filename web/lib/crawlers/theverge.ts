...
export async function crawlTheVerge(): Promise<NewsItem[]> {
  ...
  $("article a[href], a[href]").each((_, el) => {
    ...
    const root = link.closest("article, [class*='duet--item'], [class*='article'], [class*='card'], [class*='item']");
    ...
    if (!existing || (!existing.image && image)) {
      found.set(url, { id: `the-verge-${Buffer.from(url).toString("base64url")}`, source: "The Verge", category: "english", title, subtitle: subtitle && subtitle !== title ? subtitle : undefined, url, image });
    }
  });
...
function isArticle(url: string) {
  try {
    ...
    return !/\/(authors|author|tags|tag|search|videos|video|podcasts|newsletters|about|privacy|contact)(\/|$)/i.test(path) && path.split("/").filter(Boolean).length >= 2;
  } catch { return false; }
}

function isUsableTitle(title: string) {
  return title.length >= 20 && title.length <= 240 && !/^(home|the verge|reviews|features|videos|podcasts)$/i.test(title);
}
