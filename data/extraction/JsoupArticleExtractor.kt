...
class JsoupArticleExtractor(private val timeoutMillis: Int = 20_000) : ArticleExtractor {
    ...
    private fun buildContentCandidates(document: org.jsoup.nodes.Document, theVerge: Boolean, g1: Boolean): List<Element> {
        ...
        if (g1) selectors += listOf(
            ...
            ".mc-article-body",".mc-article-body-content","[data-testid*=article-body]","[data-testid*=article-content]",
            "[class*=article-body]","[class*=article-content]","[class*=materia-conteudo]"
        )
    ...
    private fun extractJsonLdArticleBody(document: org.jsoup.nodes.Document, ge: Boolean): List<ArticleBlock> {
        ...
        document.select("script[type=application/ld+json]").forEach { script ->
            ...
            body.split(Regex("\\n{2,}"))
                ...
                .filter { it.length >= 3 }
                .forEach { result.add(ArticleBlock.Paragraph(it)) }
        }
        return result
    }
    ...
    private fun isNearDuplicate(a: String, b: String): Boolean {
        if (a == b) return true
        val shorter = minOf(a.length, b.length); val longer = maxOf(a.length, b.length)
        if (shorter < 70) return false
        if ((a.contains(b) || b.contains(a)) && shorter.toDouble() / longer >= .72) return true
        val at = a.split(" ").filter { it.length >= 3 }.toSet(); val bt = b.split(" ").filter { it.length >= 3 }.toSet()
        return at.size >= 8 && bt.size >= 8 && at.intersect(bt).size.toDouble() / minOf(at.size, bt.size) >= .88
    }

    private fun extractSubtitle(document: org.jsoup.nodes.Document, title: String): String? = listOf(
        document.select("meta[name=description]").attr("content"), document.select("meta[property=og:description]").attr("content"), document.select("meta[name=twitter:description]").attr("content")
    ).map { it.replace(Regex("\\s+"), " ").trim() }.firstOrNull { it.isNotBlank() && !sameText(it, title) }

    private fun removeDuplicateLead(blocks: List<ArticleBlock>, title: String, subtitle: String?): List<ArticleBlock> {
        var removed = false
        return blocks.filter { block ->
            if (block !is ArticleBlock.Paragraph || removed) return@filter true
            val text = block.text.text
            if (!subtitle.isNullOrBlank() && (sameText(text, subtitle) || isSubtitlePrefix(text, subtitle))) { removed = true; false } else true
        }.filterNot { it is ArticleBlock.Paragraph && sameText(it.text.text, title) }
    }
    ...
    private fun addImage(result: MutableList<ArticleBlock>, image: Element, caption: String?) {
        ...
        val cleanCaption = caption?.trim()?.takeIf(String::isNotBlank)?.takeUnless(::isAccessibilityImageCaption)
        val cleanAlt = image.attr("alt").trim().takeIf(String::isNotBlank)?.takeUnless(::isAccessibilityImageCaption)
        result.add(ArticleBlock.Image(src, cleanCaption, cleanAlt))
    }

    private fun isAccessibilityImageCaption(value: String): Boolean {
        val normalized = value.replace(Regex("\\s+"), " ").trim().lowercase()
        return normalized.startsWith("imagem que representa a notícia") ||
            ...
            normalized == "imagem da notícia" || normalized == "imagem da noticia"
    }
    ...
    private fun extractPublishedAt(document: org.jsoup.nodes.Document): Instant? {
        ...
        return document.select("script[type=application/ld+json]").asSequence().flatMap { DATE_PUBLISHED_REGEX.findAll(it.data()).asSequence() }.mapNotNull { parseDate(it.groupValues[1]) }.firstOrNull()
    }
