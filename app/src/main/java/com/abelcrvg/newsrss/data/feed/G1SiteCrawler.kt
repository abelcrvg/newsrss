...
/** Reads G1 directly, combining the homepage's featured stories with the latest plantao items. */
class G1SiteCrawler {
    suspend fun crawl(source: FeedSource): Result<List<FeedItem>> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            // Keep the stories explicitly featured on the G1 homepage.
            runCatching { fetch(base) }.onSuccess { documents += it to true }

            // Add the latest stories from plantao so the feed does not depend only on
            // what happens to be featured on the homepage at refresh time.
            ...
            plantaoUrls.forEach { url ->
                runCatching { fetch(url) }.onSuccess { documents += it to false }
            }

            val homepage = documents.firstOrNull { it.second }
            val homepageItems = homepage?.let { extract(it.first, source, baseHost, true) }.orEmpty()
    ...
    private fun extract(document: Document, source: FeedSource, baseHost: String, homepage: Boolean): List<FeedItem> {
        return document.select("a[href]").mapNotNull { link ->
            ...
            if (host != baseHost || !uri.path.orEmpty().startsWith("/noticia/")) return@mapNotNull null

            val context = link.closest("article")
                ?: link.closest("[class*=feed-post], [class*=feed-item], [class*=card], [class*=story], [class*=headline], [class*=noticia], [class*=materia]")
                ?: link.parent()
    ...
    private fun extractDate(link: Element, context: Element, document: Document, url: String): Instant? {
        ...
        Regex("(20\\d{2}[-/]\\d{2}[-/]\\d{2})(?:[T/-](\\d{2}[-:]\\d{2}))?").find(url)?.let { match ->
            val value = "${match.groupValues[1].replace('/', '-')}T${match.groupValues.getOrNull(2)?.replace('-', ':') ?: "00:00"}:00"
    ...
    private companion object {
        ...
        const val MAX_HOMEPAGE_ITEMS = 40
