...
class JsoupArticleExtractor(private val timeoutMillis: Int = 20_000) : ArticleExtractor {
    ...
    private fun extractBlocks(root: Element): List<ArticleBlock> {
        ...
        copy.select("p,h2,h3,h4,h5,blockquote,ul,ol,figure,img").forEach { element -> when (element.tagName()) {
            ...
            "figure" -> element.selectFirst("img")?.let { image -> val url = imageUrl(image); if (!isNoiseImage(image) && url.isNotBlank() && seenImages.add(normalizeImageUrl(url))) result += ArticleBlock.Image(url, element.selectFirst("figcaption")?.text(), image.attr("alt")) }
            "img" -> if (element.parent()?.tagName() != "figure") { val url = imageUrl(element); if (!isNoiseImage(element) && url.isNotBlank() && seenImages.add(normalizeImageUrl(url))) result += ArticleBlock.Image(url, null, element.attr("alt")) }
        } }
    ...
    private fun extractJsonLdBody(document: Document): List<ArticleBlock> {
        ...
        document.select("script[type=application/ld+json]").forEach { script ->
            ...
            body.split(Regex("\\n{2,}"), RegexOption.IGNORE_CASE).map { it.replace(Regex("\\s+"), " ").trim() }.filter { it.length >= 2 }.forEach { result += ArticleBlock.Paragraph(it) }
        }
        return result
    }
    ...
    private fun dedupe(blocks: List<ArticleBlock>): List<ArticleBlock> { val seen = linkedSetOf<String>(); return blocks.filter { block -> val key = when (block) { is ArticleBlock.Paragraph -> normalize(block.text.text); is ArticleBlock.Heading -> "h:" + normalize(block.text); is ArticleBlock.Quote -> "q:" + normalize(block.text); is ArticleBlock.ListBlock -> "l:" + block.items.joinToString("|") { normalize(it) }; is ArticleBlock.Image -> "i:" + normalizeImageUrl(block.url) }; key.isNotBlank() && seen.add(key) } }
    ...
    private fun imageUrl(image: Element): String = listOf(image.attr("src"),image.attr("data-src"),image.attr("data-lazy-src"),image.attr("data-original"),image.attr("data-srcset").substringBefore(',')).firstOrNull(String::isNotBlank).orEmpty().let { runCatching { URI(image.baseUri()).resolve(it).toString() }.getOrDefault(it) }
    ...
    private fun isNoiseImage(image: Element): Boolean { val value = "${image.className()} ${image.id()} ${image.attr("alt")} ${image.attr("src")}".lowercase(); return listOf("avatar","author","profile","headshot","portrait","logo","icon","sprite","favicon","tracking","pixel","qr-code","qrcode","social","share","newsletter","related","recommend","thumbnail").any(value::contains) }
