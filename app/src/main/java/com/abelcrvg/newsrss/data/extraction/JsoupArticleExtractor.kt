...
class JsoupArticleExtractor(private val timeoutMillis: Int = 20_000) : ArticleExtractor {
    ...
    private fun extractBlocks(root: Element): List<ArticleBlock> {
        ...
        copy.select("p,h2,h3,h4,h5,blockquote,ul,ol,figure,img").forEach { element -> when (element.tagName()) {
            ...
            "figure" -> element.selectFirst("img")?.let { image -> val imageUrlValue = imageUrl(image); if (!isNoiseImage(image) && imageUrlValue.isNotBlank() && seenImages.add(normalizeImageUrl(imageUrlValue))) result += ArticleBlock.Image(imageUrlValue, element.selectFirst("figcaption")?.text(), image.attr("alt")) }
            "img" -> if (element.parent()?.tagName() != "figure") { val imageUrlValue = imageUrl(element); if (!isNoiseImage(element) && imageUrlValue.isNotBlank() && seenImages.add(normalizeImageUrl(imageUrlValue))) result += ArticleBlock.Image(imageUrlValue, null, element.attr("alt")) }
        } }
