...
/** Robust reader-mode extraction for Portuguese and English publishers. */
class JsoupArticleExtractor(private val timeoutMillis: Int = 20_000) : ArticleExtractor {
    override suspend fun extract(url: String): Result<Article> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val document = Jsoup.connect(url)
                ...
                .header("Accept-Language", "en-US,en;q=0.9,pt-BR;q=0.7,pt;q=0.6")
                .get()
            ...
            blocks = dedupe(blocks).let { removeLead(it, title, subtitle) }
            ...
            Article(id = url.hashCode().toUInt().toString(16), sourceId = host, url = url, title = title, subtitle = subtitle, author = extractAuthor(document), publishedAt = extractPublishedAt(document), heroImageUrl = extractHeroImage(document), blocks = blocks)
        }
    }
    ...
    private fun extractBlocks(root: Element): List<ArticleBlock> {
        ...
        copy.select("p,h2,h3,h4,h5,blockquote,ul,ol,figure,img").forEach { element ->
            when (element.tagName()) {
                ...
                "ul","ol" -> { val items = element.children().filter { it.tagName() == "li" }.map { it.text().replace(Regex("\\s+"), " ").trim() }.filter(String::isNotBlank); if (items.isNotEmpty()) result += ArticleBlock.ListBlock(items, element.tagName() == "ol") }
    ...
    private fun dedupe(blocks: List<ArticleBlock>): List<ArticleBlock> {
        ...
        return blocks.filter { block ->
            val key = when (block) { is ArticleBlock.Paragraph -> normalize(block.text.text); is ArticleBlock.Heading -> "h:" + normalize(block.text); is ArticleBlock.Quote -> "q:" + normalize(block.text); is ArticleBlock.ListBlock -> "l:" + block.items.joinToString("|") { normalize(it) }; is ArticleBlock.Image -> "i:" + block.url }
            key.isNotBlank() && seen.add(key)
        }
    }

    private fun removeLead(blocks: List<ArticleBlock>, title: String, subtitle: String?): List<ArticleBlock> = blocks.filterNot { block -> block is ArticleBlock.Paragraph && (sameText(block.text.text, title) || (!subtitle.isNullOrBlank() && sameText(block.text.text, subtitle))) }
    private fun contentScore(blocks: List<ArticleBlock>): Int = blocks.sumOf { textOf(it).length } + blocks.count { it is ArticleBlock.Paragraph } * 150
    ...
    private fun sanitizeInlineHtml(element: Element): String? {
        ...
        copy.select("*").forEach { node ->
            ...
            node.clearAttributes(); attrs.forEach { attr -> if (attr.key == "style") sanitizeStyle(attr.value).takeIf(String::isNotBlank)?.let { node.attr("style", it) } else node.attr(attr.key, attr.value) }
        }
