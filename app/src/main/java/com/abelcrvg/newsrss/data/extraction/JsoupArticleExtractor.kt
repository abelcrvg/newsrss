...
class JsoupArticleExtractor(private val timeoutMillis: Int = 20_000) : ArticleExtractor {
    override suspend fun extract(url: String): Result<Article> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            Article(id = url.hashCode().toUInt().toString(16), sourceId = URI(url).host.orEmpty(), url = url, title = title,
                ...
                heroImageUrl = firstNonBlank(document.select("meta[property=og:image]").attr("content"), document.select("meta[name=twitter:image]").attr("content")), blocks = blocks.distinct())
        }
    }
    ...
    private fun extractTheVergeBlocks(document: org.jsoup.nodes.Document): List<ArticleBlock> {
        ...
        document.select(".duet--article--article-body-component").forEach { component -> extractBlocks(component, 1).forEach { if (it !in result) result += it } }
        return result
    }
    ...
    private fun buildContentCandidates(document: org.jsoup.nodes.Document, theVerge: Boolean): List<Element> {
        val selectors = listOf("article", "main", "[role=main]", ".article-body", ".article-content", ".article__body", ".article__content", ".story-body", ".story-content", ".post-content", ".entry-content", ".content-body", ".materia-conteudo", ".materia-corpo", ".article__text", ".article-text", ".content", ".main-content", ".single-content", ".post-body", ".story-body-content", "[itemprop=articleBody]")
        ...
        document.select("div,section").asSequence().filter { it.select("p").size >= 2 && it.text().length >= 80 }.sortedByDescending(::score).take(12).forEach { if (it !in result) result += it }
        return result
    }
    ...
    private fun extractBlocks(root: Element, minParagraphLength: Int): List<ArticleBlock> {
        ...
        root.select("p,h2,h3,h4,blockquote,ul,ol,figure,img").forEach { element ->
            when (element.tagName()) {
                ...
                "ul", "ol" -> { val items = element.children().filter { it.tagName() == "li" }.map { it.text().trim() }.filter { it.isNotBlank() }; if (items.isNotEmpty()) result += ArticleBlock.ListBlock(items, element.tagName() == "ol") }
    ...
    private fun textOf(block: ArticleBlock): String = when (block) { is ArticleBlock.Paragraph -> block.text; is ArticleBlock.Heading -> block.text; is ArticleBlock.Quote -> block.text; is ArticleBlock.ListBlock -> block.items.joinToString(" "); is ArticleBlock.Image -> block.caption.orEmpty() }
