...
@Composable
fun NewsRSSSettingsButton(darkMode: Boolean, fontScale: Float, fontFamilyName: String, onChanged: (Boolean, Float, String) -> Unit) {
    var open by remember { mutableStateOf(false) }
    IconButton(onClick = { open = true }) { Text("⚙", fontSize = 24.sp) }
    if (open) NewsRSSSettingsDialog(darkMode, fontScale, fontFamilyName, onChanged) { open = false }
}

@Composable
private fun NewsRSSSettingsDialog(initialDark: Boolean, initialScale: Float, initialFamily: String, onChanged: (Boolean, Float, String) -> Unit, onDismiss: () -> Unit) {
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                    Column(Modifier.weight(1f)) { Text("Modo escuro"); Text("Usar tema escuro em todo o aplicativo", fontSize = 13.sp) }
                    Switch(checked = dark, onCheckedChange = { dark = it; onChanged(dark, scale, family) })
                }
